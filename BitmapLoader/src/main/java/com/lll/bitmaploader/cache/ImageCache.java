...
import com.lll.bitmaploader.utils.ImageResizer;
import com.lll.bitmaploader.utils.PathUtils;
import com.lll.bitmaploader.utils.Utils;
...
public class ImageCache {
     ...
     * 默认磁盘缓存大小为10
     */
    private static final int DEFAULT_DISK_CACHE_SIZE = 1024 * 10;
    ...
    public static ImageCache getInstance(FragmentManager manager, ImageCacheParams cacheParams) {
        final RetainFragment mRetainFragment = findOrCreateRetainFragment(manager);
        ImageCache cache = (ImageCache) mRetainFragment.getObject();
        if (null == cache) {
    ...
    private void init(ImageCacheParams cacheParams) {
        ...
        if (mCacheParams.memoryCacheEnabled) {//开启内存缓存
        ...
        if (cacheParams.diskCacheEnabled) {//开启disk缓存
            initDiskCache();
        }
    }
    ...
    public static long getUsableSpace(File path) {
        ...
        final StatFs stats = new StatFs(path.getAbsolutePath());
        return (long) stats.getBlockSize() * (long) stats.getAvailableBlocks();
    }

    /**
     * 存储图片到lrucache和disLruCache
     *
     * @param data  图片key
     * @param value 图片
     */
    public void addBitmapToCache(String data, BitmapDrawable value) {
        ...
        if (mMemoryCache != null) {
            if (RecyclingBitmapDrawable.class.isInstance(value)) {//传进来的bitmap是否是RecyclingBitmapDrawable类型
                //通知加到缓存中
        ...
        synchronized (mDiskCacheLock) {
            if (mDiskLruCache != null) {
                ...
                try {
                    ...
                    if (snapshot == null) {
                        final DiskLruCache.Editor editor = mDiskLruCache.editor(key);
                        if (editor != null) {
                            out = editor.newOutputStream(DISK_CACHE_INDEX);//创建流
                            //将图片保存到流中
    ...
    public Bitmap getBitmapFromDiskCache(String data) {
        ...
        Bitmap bitmap = null;
        synchronized (mDiskCacheLock){
            while (mDiskCacheStarting){//已经在缓存了,暂停等待
                try {
                  mDiskCacheLock.wait();
                }catch (Exception e){
                    e.printStackTrace();
     ...
     * 专门用来retain the ImageCache object.
     */
    ...
    private static boolean canUseForInBitmap(Bitmap candidate, BitmapFactory.Options targetOptions){
        ...
        return byteCount<getBitmapSize(candidate);
    }
    ...
    /**
     * SD card(external storage) is built-in or removable.
     * @return
     */
